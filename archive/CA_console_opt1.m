...
if ~exist('freq');
freq=repmat(z,1,1,3);     
imf=imagesc(freq(x,y,1));
figure(2)
colorbar
axis equal
axis tight
% subplot(2,1,2)
 set(imf,'erasemode','none')
end  
% subplot(2,1,1)
...
% % figure(3)
% % imc=imagesc(zeros((n+2)^2,(n+2)^2));
% % colorbar
% % axis equal
% % axis tight
% % % subplot(2,1,2)
% %  set(imc,'erasemode','none')
% %  
% figure(4)
% imC=imagesc(zeros(1,20));
% colorbar
% axis equal
% axis tight
% % subplot(2,1,2)
...
binf=0:width*5;
hf=ones(1,length(binf));
h2=line(binf,hf);
ylim([0 5])
xlim([0 100])

figure(7);clf
f7=line(1:5*width,zeros(5*width,1));
ylim()

figure(8);clf
f8 = animatedline('LineStyle','None','Marker','.');
axis([0,5,0,5]);
% 
% figure(7)
% a=line(binf,hf);
...
%         m.rulelist=cell(rulemax,2);
    ...
    if store==1
        stats=zeros(haulsize,outnumber*2);
        rulelist=cell(haulsize,2);
    end
...
record=0;
    ...
    if soupnumber==soupmax
        ...
        disp(str)   
        if store==1 
        stats(rulenumber,:)=last;
        rulelist(rulenumber,:)={ruleB,ruleS};
        
        if rulenumber==haulsize;
            if storeM==1                 
            corner=['A' num2str(haulnumber*haulsize+1)];
            if haulnumber==0
                header=true;
            else
                header=false;
