...
E_dmat_V_inputos=[];
lists={'v1','v2'};
scalars={'D_input','H_input','H_pop','MI_vic',...
...
%     S_defectT(:,end)=S_defectT(:,y(1));
    ...
    I_input=-log2(P_input_old);
    I_input(isnan(I_input))=0;
    SI_inputT=I_input(S_inputT+1);
    SI_input=SI_inputT(x,y);
    SI_input_old=cat(3,SI_input,SI_input_old(:,:,1:lag));

    HD_input=entropise(D_input);
    H_input=+sum(HD_input(:));
    H_pop2input=calc_Hxy(S_pop(:),S_input(:));
    LSI_input_old=squeeze(reshape(SI_input_old,[],1,lag+1))';
    LSI_input=LSI_input_old(1,:);
